...
/**
-----------------------------------------------------------------------------------------------
STEPS FOR DESCRIPTOR SET LAYOUT
-------------------------------
1. globally declare vulkan object of type VkDescriptorSetLayout assign it to VK_NULL_HANDLE.
2. in initialize declare and call UDF createDescriptorLayout() maintaining the convension of declaring and calling it after createShaders() and before createRenderPass().
3. while writing this UDF declare memset and initialize struct VkDecriptorSetLayoutCreateInfo perticularly its 2 memmbers bindingCount and pBinding array (pBinding array of VkDescriptorSetLayoutBinding). 
4. then call vkCreateDescriptorSetLayout() with address of above initialized structure and get the required global vulkan object vkDescriptorSetLayout with its last parameter.
5. In uninitialize() call vkDestroyDescriptorSetLayout() to destory vkDescriptorSetLayout. 


STEPS FOR PIPELINE LAYOUT
-------------------------
1. Declare globally vulkan object of type VkPipelineLayout assign it to VK_NULL_HANDEL.
2. in initialize declare and call UDF createPipelineLayout() maintaining the convension of declaring and calling it after createDescriptorLayout() and before createRenderPass()
3. while writing this UDF declare memset and initialize struct VkPipelineLayoutCreateInfo with 4 important members .setLayoutCount, .pSetLayouts, .pushConstantRangeCount, .pPushConstantRanges
4. then call vkCreatePipelineLayout() with address of above initialized structure and get the required global vulkan object vkPipelineLayout with its last parameter.
5. In uninitialize() call vkDestroyPipelineLayout() to destroy vkPipelineLayout
