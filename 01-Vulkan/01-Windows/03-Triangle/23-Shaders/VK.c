...
STEPS FOR SHADERS 
-----------------
1. write shaders and compile them to SPIR-V using shader compilation tool that we recived in vulkan SDK.
2. globally declare two shader module object variables of VkShaderModule type to hold vulkan compatible vertex shader module object and fragment shader module object respectivly.
3. declare prototype of UDF createShaders() in initialize() following convension -
    after createVertexBuffer() and before createRenderPass().
4. using same above convension call createShader between the calls of above two.
5. maintaing the same above convension while defining createShaders() between defination of above two.
6. inside createShaders() first open the shader file, set the file pointer at end of file, find the byte size of shader file data, reset the file pointer at the beginning of the file, allocate a character buffer of file size and read shader file data into it, and finally close the file, do all these thing using convensional file i/o.
7. declare and memset struct VkShaderModuleCreateInfo and specify above file size and buffer while initializing it.
8. call vkCreateShaderModule() vulkan api, pass above struct pointer to it as parameter and obtain shader module object in global variable that we declare in step 2.
9. free the shader code buffer it we allocated in step 6.
10. Assuming we did above 4 steps (6 to 9) for vertex shader repat them all for fragment shader to.
11. In uninitialize() destroy both global shader object using vkDestroyShaderModule().
...
-----------------------------------------------------------------------------------------------
 ...
 // command for magic number of SPIR-V
 Format-Hex -path .\Shader.vert.spv | Select-Object -First 1
 
 Format-Hex : give me hexadecimal format of given file path
 Select-Object : give 1st line of that hexadecimal format
