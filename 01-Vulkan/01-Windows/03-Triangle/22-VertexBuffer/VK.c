...
VkResult fillValidationLayerNames(void)
{
    ...
    fprintf(gpFile, "validation layer count = %d\n\n",validationLayerCount);
    if (vkResult != VK_SUCCESS)
    {
...
Vertex Buffer
-------------
1. Declare globally a sturcture holding vertex buffer related 2 things VkBuffer object, VkDeviceMemory object we will call it struct VertexData and declare global variable of this structure named vertexData_position. 
2. Declare UDF createVertexBuffer() write its prototype below createCommandBuffer() and above createRenderPass() and also call between the calls of these two.
3. implement this UDF and inside it first declare our triangles position array. 
4. memset our global vertexData_position struct. 
5. declare and memset struct VkBufferCreateInfo it has 8 member we will use 5 out of them 2 is very importent usage, size. 
6. call vkCreateBuffer() vulkan api in the .vkBuffer member of our global struct.
7. declare and memset struct VkMemoryRequirements and the call vkGetBufferMemeoryRequirements() to get memory requirement. 
8. To actually allocate the require memeory we need to call vkAllocateMemory() but before that we need to declare and memset VkMemoryAllocateInfo sturct, important memeber of this sturcture are memoryTypeIndex and allocationSize for allocation size use the size obtain from memory requirement for memeoryTypeIndex 
    a. start a loop for with counter as vkPhysicalDeviceMemoryProperties.memoryTypeCount
    b. inside the loop check vkMemoryRequirements.memoryTypeBits contains 1 or not if yes 
    c. check vkPhyscialDeviceMemoryProperties.memoryTypes[i].propertyFlag memeber contains VK_MEMEORY_PROPERTY_HOST_VISIBLE_BIT if yes. 
    d. then this ith index will be our memeory type index if found break out of loop.
    e. if not continue the loop by right shifting memoryRequirements.memoryTypeBites by 1 over each iteration. 
9. call vkAllocateMemory() and get the required vulkan memory handle into the .vkDeviceMemory of our global structure. 
10. Now we have our required device memeory handle and vkDevice handle
11. bind this deivce memory handle to vulkan memeory handle by using vkBindBufferMemory() declare local void *data (buffer) and call vkMapMemory() to map our device memory object handle to this void *data this will allow us to do memory mapped i/o when we write in void *data it will atomatically written or copyed on the device memory represented by deviceMemory object handle. 
12. Now do actually memory maped i/o call memcpy(). 
13. To complete this memeory maped io vkUnmappedMemory(). 
14. in uninitialize first free the vkDeviceMemory member of our global structure using vkFreeMemory() and then destroy vkBuffer of our global structure by using vkDestoryBuffer(). 
