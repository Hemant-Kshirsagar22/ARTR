...
void uninitialize(void)
{
    ...
    // freeing vertexData_position_index
    if(vertexData_position_index.vkDeviceMemory)
    {
        ...
        fprintf(gpFile, "%s()-> vertexData_position_index.vkDeviceMemory is freed\n", __func__);
    }
    ...
    if(vertexData_position_index.vkBuffer)
    {
        ...
        fprintf(gpFile, "%s()-> vertexData_position_index.vkBuffer is freed\n", __func__);
    }
...
VkResult createVertexBuffer(void)
{
    ...
    // memset our global vertexData_position_index struct.
    ...
    vkResult = vkAllocateMemory(vkDevice, &vkMemeoryAllocateInfo, NULL, &vertexData_position_index.vkDeviceMemory);

    if (vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s()-> vkAllocateMemory() for vertexData_position_index.vkDeviceMemory failed !!!\n\n", __func__);
        return (vkResult);
    }
    else
    {
        fprintf(gpFile, "%s()-> vkAllocateMemory() call success for vertexData_position_index.vkDeviceMemory\n\n", __func__);
    }

    vkResult = vkBindBufferMemory(vkDevice, vertexData_position_index.vkBuffer, vertexData_position_index.vkDeviceMemory, 0);
    if (vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s()-> vkBindBufferMemory() failed for vertexData_position_index!!!\n\n", __func__);
        return (vkResult);
    }
    else
    {
        fprintf(gpFile, "%s()-> vkBindBufferMemory() call success for vertexData_position_index\n\n", __func__);
    }

    data = NULL;
    vkResult = vkMapMemory(vkDevice, vertexData_position_index.vkDeviceMemory, 0, vkMemeoryAllocateInfo.allocationSize, 0, &data);
    if (vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s()-> vkMapMemory() failed for vertexData_position_index!!!\n\n", __func__);
        return (vkResult);
    }
    else
    {
        fprintf(gpFile, "%s()-> vkMapMemory() call success for vertexData_position_index\n\n", __func__);
    }
...
VkResult buildCommandBuffers(void)
{
    ...
    for(uint32_t i = 0; i < swapchainImageCount; i++)
    {
        ...
        vkCmdBindIndexBuffer(vkCommandBuffer_array[i], vertexData_position_index.vkBuffer, 0, VK_INDEX_TYPE_UINT32); // 4th parameter should mactch with our triangle_position_indices
