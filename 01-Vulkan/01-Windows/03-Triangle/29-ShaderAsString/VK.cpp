...
// GLSLang and SPIR-V related libraries
...
VkResult createShaders(void)
{
    ...
    {
        fprintf(gpFile, "%s()-> glsl2spv() success for fragment shader \n\n", __func__);
    }
...
BOOL glsl2spv(const VkShaderStageFlagBits shader_type, const char *pShader, vector<unsigned int> &spirv)
{
    ...
    // create TShader type Shader from above EShLanguage by calling TShader constructor;
	glslang::TShader shader(language);

    // put our shader source code into shader array
	...
	{
        fprintf(gpFile, "%s()-> program.link() call failed !!!\nLOG : %s\n\n", __func__, shader.getInfoLog());
        fprintf(gpFile, "%s()-> DEBUG LOG : %s\n\n", __func__, shader.getInfoDebugLog());
		return(FALSE);
	}
    else
    {
        fprintf(gpFile, "%s()-> program.link() call success\n\n", __func__);
    }

	// now save this whole cooked program and spirv program to glslang library
...
/**
-----------------------------------------------------------------------------------------------
Notes for Shader as String
we are going to use GLSLang and SPIR-V library which are wrtten in C++.
* In this code we are adding some headers that will change at every version update of vulkan.
