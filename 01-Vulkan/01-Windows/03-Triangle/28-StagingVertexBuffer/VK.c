...
VkResult createVertexBuffer(void)
{
    ...
    // code
    fprintf(gpFile, "\n======================== CREATE VERTEX BUFFER START (STAGING BUFFER)================================\n\n");
    ...
    if (vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s()-> vkCreateBuffer() failed for creating vertex staging buffer!!!\n\n", __func__);
        return (vkResult);
    }
    else
    {
    ...
    vkResult = vkAllocateMemory(vkDevice, &vkMemeoryAllocateInfo_stagingBuffer, NULL, &vertexData_stagingBuffer_position.vkDeviceMemory);

    if (vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s()-> vkAllocateMemory() for vertexData_stagingBuffer_position.vkDeviceMemory failed !!!\n\n", __func__);
        return (vkResult);
    }
    else
    {
        fprintf(gpFile, "%s()-> vkAllocateMemory() call success for vertexData_stagingBuffer_position.vkDeviceMemory\n\n", __func__);
    }
    ...
    void *data = NULL;
    vkResult = vkMapMemory(vkDevice, vertexData_stagingBuffer_position.vkDeviceMemory, 0, vkMemeoryAllocateInfo_stagingBuffer.allocationSize, 0, &data);
    if (vkResult != VK_SUCCESS)
    {
    ...
    if (vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s()-> vkAllocateCommandBuffers() failed  for buffer copy (ERROR CODE : %d )!!!\n\n", __func__, vkResult);
        return (vkResult);
    }
    else
    {
        fprintf(gpFile, "%s()-> vkAllocateCommandBuffers() call success for buffer copy\n\n", __func__);
    }
    ...
    if(vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s()-> vkBeginCommandBuffer() failed for buffer copy (ERROR CODE : %d)\n\n", __func__, vkResult);
        return(vkResult);
    }
    else
    {
        fprintf(gpFile, "%s()-> vkBeginCommandBuffer() success for buffer copy\n", __func__);
    }
    ...
    if(vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s()-> vkEndCommandBuffer() failed for buffer copy (ERROR CODE : %d)\n\n", __func__, vkResult);
        return(vkResult);
    }
    else
    {
        fprintf(gpFile, "%s()-> vkEndCommandBuffer() success for buffer copy\n", __func__);
    }
    ...
    if(vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s()-> vkQueueSubmit() failed for buffer copy (ERROR CODE : %d)\n\n", __func__, vkResult);
        return(vkResult);
    }
    else
    {
        fprintf(gpFile, "%s()-> vkQueueSubmit() success for buffer copy\n", __func__);
    }
    ...
    if(vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s()-> vkQueueWaitIdle() failed for buffer copy (ERROR CODE : %d)\n\n", __func__, vkResult);
        return(vkResult);
    }
    else
    {
        fprintf(gpFile, "%s()-> vkQueueWaitIdle() success for buffer copy\n", __func__);
    }
    ...
    if(vkCommandBuffer)
    {
        ...
        fprintf(gpFile, "vkCommandBuffer is freed for buffer copy\n\n");
    }
    ...
    if(vertexData_stagingBuffer_position.vkDeviceMemory)
    {
        ...
        fprintf(gpFile, "%s()-> vertexData_stagingBuffer_position.vkDeviceMemory is freed\n", __func__);
    }
    ...
    if(vertexData_stagingBuffer_position.vkBuffer)
    {
        ...
        fprintf(gpFile, "%s()-> vertexData_stagingBuffer_position.vkBuffer is freed\n", __func__);
    }
...
-----------------------------------------------------------------------------------------------
 ...
 ONLY CREATE VERTEX BUFFER IS CHANGED THIS IS NOT THE STEP FOR RENDERING 
 Staging Vertex Buffer
 ---------------
 1. Create a local vertex buffer just like previously created vertex buffer,But with different usage and different sharing mode, Identify it as a staging Buffer, This buffer is visible to host and can be mapped to device memory
 2. Then Create the usual vertex buffer visible for only to device.
 3. Create one special command buffer.
 4. Build above command buffer by vulkan’s buffer copy command and copy the data in the staging buffer from step 1 to the destination buffer from step 2, While Doing this use our queue and the command buffer to submit the work of buffer copy
 5. After done free / destroy the command buffer
 6. Destroy the local staging buffer as it job is done.
