...
// for VK_KHR_SWAPCHAIN_EXTENSION_NAME and VK_KHR_EXTERNAL_MEMORY_WIN32_EXTENSION_NAME
...
cudaExternalMemory_t cuExternalMemory_for_64x64;
cudaExternalMemory_t cuExternalMemory_for_128x128;
cudaExternalMemory_t cuExternalMemory_for_256x256;
cudaExternalMemory_t cuExternalMemory_for_512x512;
cudaExternalMemory_t cuExternalMemory_for_1024x1024;

void *pos_cuda_for_64x64 = NULL;
void *pos_cuda_for_128x128 = NULL;
void *pos_cuda_for_256x256 = NULL;
void *pos_cuda_for_512x512 = NULL;
void *pos_cuda_for_1024x1024 = NULL;

VertexData vertexData_external_for_64x64;
VertexData vertexData_external_for_128x128;
VertexData vertexData_external_for_256x256;
VertexData vertexData_external_for_512x512;
VertexData vertexData_external_for_1024x1024;
BOOL bOnGPU = FALSE;
...
VkResult initialize(void)
{
    ...
    VkResult createExternalVertexBuffer(unsigned int, unsigned int, VertexData *, cudaExternalMemory_t *);
    ...
    {
        fprintf(gpFile, "%s()-> initialize_cuda() failed !!! (ERROR CODE : %d)\n\n", __func__, vkResult);
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        return (vkResult);
    }
    else
    {
        fprintf(gpFile, "%s()-> initialize_cuda() success\n\n", __func__);
    }
    ...
    if (vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s()-> createCommandBuffer(vkCommandBuffer_for_1024x1024_graphics_array) failed !!! (ERROR CODE : %d)\n\n", __func__, vkResult);
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        return (vkResult);
    }
    else
    ...
    if (vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s()-> createVertexBuffer() failed for vertexData_position_1024x1024_graphics !!! (ERROR CODE : %d)\n\n", __func__, vkResult);
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        return (vkResult);
    }
    else
    ...
    // create external vertex buffer for 64x64
    memset((void *)&vertexData_external_for_64x64, 0, sizeof(VertexData));
    vkResult = createExternalVertexBuffer(64, 64, &vertexData_external_for_64x64, &cuExternalMemory_for_64x64);
    if (vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s()-> createExternalVertexBuffer() failed for vertexData_external_for_64x64 !!! (ERROR CODE : %d)\n\n", __func__, vkResult);
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        return (vkResult);
    }
    else
    {
        fprintf(gpFile, "%s()-> createExternalVertexBuffer() success for vertexData_external_for_64x64 \n\n", __func__);
    }

    // create external vertex buffer for 128x128
    memset((void *)&vertexData_external_for_128x128, 0, sizeof(VertexData));
    vkResult = createExternalVertexBuffer(128, 128, &vertexData_external_for_128x128, &cuExternalMemory_for_128x128);
    if (vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s()-> createExternalVertexBuffer() failed for vertexData_external_for_128x128 !!! (ERROR CODE : %d)\n\n", __func__, vkResult);
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        return (vkResult);
    }
    else
    {
        fprintf(gpFile, "%s()-> createExternalVertexBuffer() success for vertexData_external_for_128x128\n\n", __func__);
    }

    // create external vertex buffer for 256x256
    memset((void *)&vertexData_external_for_256x256, 0, sizeof(VertexData));
    vkResult = createExternalVertexBuffer(256, 256, &vertexData_external_for_256x256, &cuExternalMemory_for_256x256);
    if (vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s()-> createExternalVertexBuffer() failed for vertexData_external_for_256x256 !!! (ERROR CODE : %d)\n\n", __func__, vkResult);
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        return (vkResult);
    }
    else
    {
        fprintf(gpFile, "%s()-> createExternalVertexBuffer() success for vertexData_external_for_256x256\n\n", __func__);
    }

    // create external vertex buffer for 512x512
    memset((void *)&vertexData_external_for_512x512, 0, sizeof(VertexData));
    vkResult = createExternalVertexBuffer(512, 512, &vertexData_external_for_512x512, &cuExternalMemory_for_512x512);
    if (vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s()-> createExternalVertexBuffer() failed for vertexData_external_for_512x512 !!! (ERROR CODE : %d)\n\n", __func__, vkResult);
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        return (vkResult);
    }
    else
    {
        fprintf(gpFile, "%s()-> createExternalVertexBuffer() success for vertexData_external_for_512x512\n\n", __func__);
    }

    // create external vertex buffer for 1024x1024
    memset((void *)&vertexData_external_for_1024x1024, 0, sizeof(VertexData));
    vkResult = createExternalVertexBuffer(1024, 1024, &vertexData_external_for_1024x1024, &cuExternalMemory_for_1024x1024);
    if (vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s()-> createExternalVertexBuffer() failed for vertexData_external_for_1024x1024 !!! (ERROR CODE : %d)\n\n", __func__, vkResult);
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        return (vkResult);
    }
    else
    {
        fprintf(gpFile, "%s()-> createExternalVertexBuffer() success for vertexData_external_for_1024x1024\n\n", __func__);
    }
...
cudaError_t initialize_cuda(void)
{
	...
	else // SUCCESS
	{
        ...
        for(int i = 0; i < devCount; i++)
        {
            ...
            else if(compute_mode == cudaComputeModeProhibited)
            {
                continue;
            }
            ...
            // required device is found now check it
            ...
            {
                fprintf(gpFile, "%s() -> selected device : %s\n\n", __func__, devProp.name);
        ...
        {
            fprintf(gpFile, "%s() -> vulkan cuda interOp device not found !!!\n\n", __func__);
            return(cudaErrorUnknown);
        }
	}

    // assuming we are already using windows os greater than 8.1 but with win32 application not metro application
...
VkResult resize(int width, int height)
{
    ...
    for (uint32_t i = 0; i < swapchainImageCount; i++)
    {
        ...
        vkCommandBuffer_for_1024x1024_graphics_array[i] = NULL;
    }
...
VkResult display(void)
{
    ...
    switch(selected_mesh_size)
    {
        case MESH_SIZE_64X64:
            ...
            else
            {
	            ...
	            dim3 grid = dim3(64 / block.x, 64 / block.y, 1);
	            sineWaveKernel << <grid, block >> > ((float4 *)pos_cuda_for_64x64, 64, 64, animation_time);
            }
            break;
        case MESH_SIZE_128X128:
            ...
            {
                dim3 block = dim3(8, 8, 1);
	            dim3 grid = dim3(128 / block.x, 128 / block.y, 1);
	            sineWaveKernel << <grid, block >> > ((float4 *)pos_cuda_for_128x128, 128, 128, animation_time);
            }
            break;
        case MESH_SIZE_256X256:
            ...
            {
                dim3 block = dim3(8, 8, 1);
	            dim3 grid = dim3(256 / block.x, 256 / block.y, 1);
	            sineWaveKernel << <grid, block >> > ((float4 *)pos_cuda_for_256x256, 256, 256, animation_time);
            }
            break;
        case MESH_SIZE_512X512:
            ...
            {
                dim3 block = dim3(8, 8, 1);
	            dim3 grid = dim3(512 / block.x, 512 / block.y, 1);
	            sineWaveKernel << <grid, block >> > ((float4 *)pos_cuda_for_512x512, 512, 512, animation_time);
            }
            break;
        case MESH_SIZE_1024X1024:
            ...
            else
            {
	            ...
	            sineWaveKernel << <grid, block >> > ((float4 *)pos_cuda_for_1024x1024, 1024, 1024, animation_time);
            }
            break;
        default:
            if(bOnGPU == FALSE)
    ...
    // common for all mesh size
    if(bOnGPU == TRUE)
    {
        cudaResult = cudaGetLastError();
        if(cudaResult != cudaSuccess)
        {
            fprintf(gpFile, "%s()-> cudaGetLastError() failed !!!\n\n", __func__);
            vkResult = VK_ERROR_INITIALIZATION_FAILED;
            return(vkResult);
        }
        ...
        if(cudaResult != cudaSuccess)
        {
            fprintf(gpFile, "%s()-> cudaDeviceSynchronize() failed !!!\n\n", __func__);
            vkResult = VK_ERROR_INITIALIZATION_FAILED;
            return(vkResult);
        }
    }
...
void uninitialize(void)
{
    ...
    if(cudaResult != cudaSuccess)
    {
        fprintf(gpFile, "%s()-> uninitialize_cuda() failed !!!\n", __func__);
    }
    else
    {
        fprintf(gpFile, "%s()-> uninitialize_cuda() success !!!\n", __func__);
    }
    ...
    if(vertexData_position_1024x1024_graphics.vkDeviceMemory)
    {
        ...
        fprintf(gpFile, "%s()-> vertexData_position_1024x1024_graphics.vkDeviceMemory is freed\n", __func__);
    }
    ...
    if(vertexData_position_1024x1024_graphics.vkBuffer)
    {
        ...
        fprintf(gpFile, "%s()-> vertexData_position_1024x1024_graphics.vkBuffer is freed\n", __func__);
    }
    
    // external buffer for 64x64
    if(vertexData_external_for_64x64.vkDeviceMemory)
    {
        vkFreeMemory(vkDevice, vertexData_external_for_64x64.vkDeviceMemory, NULL);
        vertexData_external_for_64x64.vkDeviceMemory = VK_NULL_HANDLE;
        fprintf(gpFile, "%s()-> vertexData_external_for_64x64.vkDeviceMemory is freed\n", __func__);
    }

    if(vertexData_external_for_64x64.vkBuffer)
    {
        vkDestroyBuffer(vkDevice, vertexData_external_for_64x64.vkBuffer, NULL);
        vertexData_external_for_64x64.vkBuffer = VK_NULL_HANDLE;
        fprintf(gpFile, "%s()-> vertexData_external_for_64x64.vkBuffer is freed\n", __func__);
    }

    // external buffer for 128x128
    if(vertexData_external_for_128x128.vkDeviceMemory)
    {
        vkFreeMemory(vkDevice, vertexData_external_for_128x128.vkDeviceMemory, NULL);
        vertexData_external_for_128x128.vkDeviceMemory = VK_NULL_HANDLE;
        fprintf(gpFile, "%s()-> vertexData_external_for_128x128.vkDeviceMemory is freed\n", __func__);
    }

    if(vertexData_external_for_128x128.vkBuffer)
    {
        vkDestroyBuffer(vkDevice, vertexData_external_for_128x128.vkBuffer, NULL);
        vertexData_external_for_128x128.vkBuffer = VK_NULL_HANDLE;
        fprintf(gpFile, "%s()-> vertexData_external_for_128x128.vkBuffer is freed\n", __func__);
    }

    // external buffer for 256x256
    if(vertexData_external_for_256x256.vkDeviceMemory)
    {
        vkFreeMemory(vkDevice, vertexData_external_for_256x256.vkDeviceMemory, NULL);
        vertexData_external_for_256x256.vkDeviceMemory = VK_NULL_HANDLE;
        fprintf(gpFile, "%s()-> vertexData_external_for_256x256.vkDeviceMemory is freed\n", __func__);
    }

    if(vertexData_external_for_256x256.vkBuffer)
    {
        vkDestroyBuffer(vkDevice, vertexData_external_for_256x256.vkBuffer, NULL);
        vertexData_external_for_256x256.vkBuffer = VK_NULL_HANDLE;
        fprintf(gpFile, "%s()-> vertexData_external_for_256x256.vkBuffer is freed\n", __func__);
    }

    // external buffer for 512x512
    if(vertexData_external_for_512x512.vkDeviceMemory)
    {
        vkFreeMemory(vkDevice, vertexData_external_for_512x512.vkDeviceMemory, NULL);
        vertexData_external_for_512x512.vkDeviceMemory = VK_NULL_HANDLE;
        fprintf(gpFile, "%s()-> vertexData_external_for_512x512.vkDeviceMemory is freed\n", __func__);
    }

    if(vertexData_external_for_512x512.vkBuffer)
    {
        vkDestroyBuffer(vkDevice, vertexData_external_for_512x512.vkBuffer, NULL);
        vertexData_external_for_512x512.vkBuffer = VK_NULL_HANDLE;
        fprintf(gpFile, "%s()-> vertexData_external_for_512x512.vkBuffer is freed\n", __func__);
    }
    
    // external buffer for 1024x1024
    if(vertexData_external_for_1024x1024.vkDeviceMemory)
    {
        vkFreeMemory(vkDevice, vertexData_external_for_1024x1024.vkDeviceMemory, NULL);
        vertexData_external_for_1024x1024.vkDeviceMemory = VK_NULL_HANDLE;
        fprintf(gpFile, "%s()-> vertexData_external_for_1024x1024.vkDeviceMemory is freed\n", __func__);
    }

    if(vertexData_external_for_1024x1024.vkBuffer)
    {
        vkDestroyBuffer(vkDevice, vertexData_external_for_1024x1024.vkBuffer, NULL);
        vertexData_external_for_1024x1024.vkBuffer = VK_NULL_HANDLE;
        fprintf(gpFile, "%s()-> vertexData_external_for_1024x1024.vkBuffer is freed\n", __func__);
    }
...
cudaError_t uninitialize_cuda(void)
{
    if(pos_cuda_for_64x64)
    {
        cudaResult = cudaFree(pos_cuda_for_64x64);
        if(cudaResult != cudaSuccess)
        {
        ...
        else
        {
            ...
            pos_cuda_for_64x64 = NULL;
        }
    }

    if(pos_cuda_for_128x128)
    {
        cudaResult = cudaFree(pos_cuda_for_128x128);
        if(cudaResult != cudaSuccess)
        {
            fprintf(gpFile, "%s() -> cudaFree() failed !!!", __func__);
            return(cudaResult);
        ...
        {
            fprintf(gpFile, "%s() -> cudaFree() success !!!", __func__);
            pos_cuda_for_128x128 = NULL;
        }
    }
    
    if(pos_cuda_for_256x256)
    {
        cudaResult = cudaFree(pos_cuda_for_256x256);
        if(cudaResult != cudaSuccess)
        {
            fprintf(gpFile, "%s() -> cudaFree() failed !!!", __func__);
            return(cudaResult);
        ...
        {
            fprintf(gpFile, "%s() -> cudaFree() success !!!", __func__);
            pos_cuda_for_256x256 = NULL;
        }
    }

    if(pos_cuda_for_512x512)
    {
        cudaResult = cudaFree(pos_cuda_for_512x512);
        if(cudaResult != cudaSuccess)
        {
            fprintf(gpFile, "%s() -> cudaFree() failed !!!", __func__);
            return(cudaResult);
        ...
        {
            fprintf(gpFile, "%s() -> cudaFree() success !!!", __func__);
            pos_cuda_for_512x512 = NULL;
        }
    }

    if(pos_cuda_for_1024x1024)
    {
        cudaResult = cudaFree(pos_cuda_for_1024x1024);
        if(cudaResult != cudaSuccess)
        {
            fprintf(gpFile, "%s() -> cudaFree() failed !!!", __func__);
            return(cudaResult);
        ...
        {
            fprintf(gpFile, "%s() -> cudaFree() success !!!", __func__);
            pos_cuda_for_1024x1024 = NULL;
        }
    }

    if(cuExternalMemory_for_64x64)
    {
        cudaResult = cudaDestroyExternalMemory(cuExternalMemory_for_64x64);
        if(cudaResult != cudaSuccess)
        {
        ...
        cuExternalMemory_for_64x64 = NULL;
    }

    if(cuExternalMemory_for_128x128)
    {
        cudaResult = cudaDestroyExternalMemory(cuExternalMemory_for_128x128);
        if(cudaResult != cudaSuccess)
        {
            fprintf(gpFile, "%s() -> cudaDestroyExternalMemory() failed !!!", __func__);
            return(cudaResult);
        ...
        cuExternalMemory_for_128x128 = NULL;
    }
    
        if(cuExternalMemory_for_256x256)
        {
            cudaResult = cudaDestroyExternalMemory(cuExternalMemory_for_256x256);
            if(cudaResult != cudaSuccess)
            {
                fprintf(gpFile, "%s() -> cudaDestroyExternalMemory() failed !!!", __func__);
                return(cudaResult);
            ...
            cuExternalMemory_for_256x256 = NULL;
        }

    if(cuExternalMemory_for_512x512)
    {
        cudaResult = cudaDestroyExternalMemory(cuExternalMemory_for_512x512);
        if(cudaResult != cudaSuccess)
        {
            fprintf(gpFile, "%s() -> cudaDestroyExternalMemory() failed !!!", __func__);
            return(cudaResult);
        ...
        cuExternalMemory_for_512x512 = NULL;
    }

    if(cuExternalMemory_for_1024x1024)
    {
        cudaResult = cudaDestroyExternalMemory(cuExternalMemory_for_1024x1024);
        if(cudaResult != cudaSuccess)
        {
            fprintf(gpFile, "%s() -> cudaDestroyExternalMemory() failed !!!", __func__);
            return(cudaResult);
        ...
        cuExternalMemory_for_1024x1024 = NULL;
    }
...
VkResult createCommandBuffer(VkCommandBuffer **ppVkCommandBuffer_array)
{
    // variable declarations
    VkResult vkResult = VK_SUCCESS;
    VkCommandBuffer *vkCommandBuffer_array = NULL;
...
VkResult createExternalVertexBuffer(unsigned int mesh_width, unsigned int mesh_height, VertexData *pVertexData, cudaExternalMemory_t *pCuExternalMemory)
{
    // local variable declarations
    VkResult vkResult = VK_SUCCESS;
    VertexData vertexData_position;
    ...
    {
        fprintf(gpFile, "%s() -> vkGetMemoryWin32HandleKHR is NULL !!!", __func__);
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        return(vkResult);
    }
    else
    {
        fprintf(gpFile, "%s() -> vkGetMemoryWin32HandleKHR get success !!!", __func__);
    }

    vkResult = vkGetMemoryWin32HandleKHR(vkDevice, &vkMemoryGetWin32HandleInfoKHR, &hMemoryWin32Handle);
    if(vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s() -> vkGetMemoryWin32HandleKHR() failed !!!", __func__);
        return(vkResult);
    }
    else
    {
        fprintf(gpFile, "%s() -> vkGetMemoryWin32HandleKHR() success", __func__);
    }

    // import above external buffer memory into cuda
    ...
    cuExternalMemoryHandleDesc.type = cudaExternalMemoryHandleTypeOpaqueWin32; // cuda inbuild constant for windows 8.1+
    ...
    if(cudaResult != cudaSuccess)
    {
        fprintf(gpFile, "%s() -> cudaImportExternalMemory() failed\n\n", __func__);
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
    }
    else
    {
        fprintf(gpFile, "%s() -> cudaImportExternalMemory() success\n\n", __func__);
    }
    ...
    if(mesh_width == 64 && mesh_height == 64)
    {        
        cudaResult = cudaExternalMemoryGetMappedBuffer(&pos_cuda_for_64x64, cuExternalMemory, &cuExternalMemoryBufferDesc);
        if(cudaResult != cudaSuccess)
        {
        ...
        {
            fprintf(gpFile, "%s() -> cudaExternalMemroyGetMappedBuffer() success\n\n", __func__);
        }
    }
    else if(mesh_width == 128 && mesh_height == 128)
    {
        cudaResult = cudaExternalMemoryGetMappedBuffer(&pos_cuda_for_128x128, cuExternalMemory, &cuExternalMemoryBufferDesc);
        if(cudaResult != cudaSuccess)
        {
            fprintf(gpFile, "%s() -> cudaExternalMemroyGetMappedBuffer() failed\n\n", __func__);
            vkResult = VK_ERROR_INITIALIZATION_FAILED;
    ...
    {
        cudaResult = cudaExternalMemoryGetMappedBuffer(&pos_cuda_for_256x256, cuExternalMemory, &cuExternalMemoryBufferDesc);
            if(cudaResult != cudaSuccess)
            {
                fprintf(gpFile, "%s() -> cudaExternalMemroyGetMappedBuffer() failed\n\n", __func__);
                vkResult = VK_ERROR_INITIALIZATION_FAILED;
    ...
    else if(mesh_width == 512 && mesh_height == 512)
    {
        ...
        cudaResult = cudaExternalMemoryGetMappedBuffer(&pos_cuda_for_512x512, cuExternalMemory, &cuExternalMemoryBufferDesc);
        if(cudaResult != cudaSuccess)
        {
            fprintf(gpFile, "%s() -> cudaExternalMemroyGetMappedBuffer() failed\n\n", __func__);
            vkResult = VK_ERROR_INITIALIZATION_FAILED;
    ...
    {
        cudaResult = cudaExternalMemoryGetMappedBuffer(&pos_cuda_for_1024x1024, cuExternalMemory, &cuExternalMemoryBufferDesc);
        if(cudaResult != cudaSuccess)
        {
            fprintf(gpFile, "%s() -> cudaExternalMemroyGetMappedBuffer() failed\n\n", __func__);
            vkResult = VK_ERROR_INITIALIZATION_FAILED;
    ...
    *pCuExternalMemory = cuExternalMemory;
    fprintf(gpFile, "\n======================== CREATE EXTERNAL VERTEX BUFFER END ================================\n\n");
    return(vkResult);
}
...
VkResult updateUniformBuffer(void)
{
    ...
    myUniformData.modelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, 0.0f));
    ...
    for(uint32_t i = 0; i < swapchainImageCount; i++)
    {
        ...
        switch(selected_mesh_size)
        {
            case MESH_SIZE_64X64:
                ...
                {
                    vkCmdBindVertexBuffers(vkCommandBuffer_array[i], 0, 1, &vertexData_external_for_64x64.vkBuffer, vkDeviceSize_offset_array);
                    vkCmdDraw(vkCommandBuffer_array[i],
                        64 * 64, // no of vertices
                        1, // no of instance
                        0, // first vertex
            ...
            case MESH_SIZE_128X128:
                ...
                {
                    vkCmdBindVertexBuffers(vkCommandBuffer_array[i], 0, 1, &vertexData_external_for_128x128.vkBuffer, vkDeviceSize_offset_array);
                    vkCmdDraw(vkCommandBuffer_array[i],
                        128 * 128, // no of vertices
                        1, // no of instance
                        0, // first vertex
            ...
            case MESH_SIZE_256X256:
                ...
                {
                    vkCmdBindVertexBuffers(vkCommandBuffer_array[i], 0, 1, &vertexData_external_for_256x256.vkBuffer, vkDeviceSize_offset_array);
                    vkCmdDraw(vkCommandBuffer_array[i],
                        256 * 256, // no of vertices
                        1, // no of instance
                        0, // first vertex
            ...
            case MESH_SIZE_512X512:
                ...
                {
                    vkCmdBindVertexBuffers(vkCommandBuffer_array[i], 0, 1, &vertexData_external_for_512x512.vkBuffer, vkDeviceSize_offset_array);
                    vkCmdDraw(vkCommandBuffer_array[i],
                        512 * 512, // no of vertices
                        1, // no of instance
                        0, // first vertex
            ...
            case MESH_SIZE_1024X1024:
                ...
                {
                    vkCmdBindVertexBuffers(vkCommandBuffer_array[i], 0, 1, &vertexData_external_for_1024x1024.vkBuffer, vkDeviceSize_offset_array);
                    vkCmdDraw(vkCommandBuffer_array[i],
                        1024 * 1024, // no of vertices
                        1, // no of instance
                        0, // first vertex
...
-----------------------------------------------------------------------------------------------
 ...
 // for cuda
 nvcc.exe -I "C:/VulkanSDK/vulkan/include" "" -L "" -o VK.exe user32.lib gdi32.lib vk.res VK.cu -diag -supperess 20012 -diag -supperess 20013 -diag -supperess 20014 -diag -supperess 20015
