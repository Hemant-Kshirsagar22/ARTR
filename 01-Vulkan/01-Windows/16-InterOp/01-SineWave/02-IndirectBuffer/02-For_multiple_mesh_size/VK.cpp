...
VkResult initialize(void)
{
    ...
    pPosition = (float *)malloc(mesh_array_size * sizeof(float));
    ...
    if (vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s()-> createIndirectBuffer() failed !!! (ERROR CODE : %d)\n\n", __func__, vkResult);
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        return (vkResult);
    }
    else
    {
        fprintf(gpFile, "%s()-> createIndirectBuffer() success \n\n", __func__);
    }
...
VkResult display(void)
{
    ...
    // reallocation of posiiton array
    pPosition = (float *)malloc(mesh_array_size * sizeof(float));
    ...
    if(vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s()-> updateVertexBuffer() failed (ERROR CODE : %d) !!!\n\n", __func__, vkResult);
    }
    ...
    if(vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s()-> updateIndirectBuffer() failed (ERROR CODE : %d) !!!\n\n", __func__, vkResult);
    }
...
void sineWave(unsigned int mesh_width, unsigned int mesh_height, float anim_time)
{
...
void uninitialize(void)
{
    ...
    if(vertexData_indirect_buffer.vkDeviceMemory)
    {
        ...
        fprintf(gpFile, "%s()-> vertexData_indirect_buffer.vkDeviceMemory is freed\n", __func__);
    }
    ...
    if(vertexData_indirect_buffer.vkBuffer)
    {
        ...
        fprintf(gpFile, "%s()-> vertexData_indirect_buffer.vkBuffer is freed\n", __func__);
    }
...
VkResult createVertexBuffer(void)
{
    ...
    if (vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s()-> updateVertexBuffer() failed !!!\n\n", __func__);
        return (vkResult);
    }
    else
    {
        fprintf(gpFile, "%s()-> updateVertexBuffer() call success\n\n", __func__);
    }
...
VkResult createIndirectBuffer(void)
{
    ...
    // memset our global vertexData_indirect_buffer struct.
    ...
    vkResult = vkAllocateMemory(vkDevice, &vkMemeoryAllocateInfo, NULL, &vertexData_indirect_buffer.vkDeviceMemory);

    if (vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s()-> vkAllocateMemory() for vertexData_indirect_buffer.vkDeviceMemory failed !!!\n\n", __func__);
        return (vkResult);
    }
    else
    {
        fprintf(gpFile, "%s()-> vkAllocateMemory() call success for vertexData_indirect_buffer.vkDeviceMemory\n\n", __func__);
    }
    ...
    if (vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s()-> updateIndirectBuffer() failed !!!\n\n", __func__);
        return (vkResult);
    }
    else
    {
        fprintf(gpFile, "%s()-> updateIndirectBuffer() call success\n\n", __func__);
    }
