...
VkResult initialize(void)
{
    ...
    // create texture
    vkResult = createTexture("marbal.png");
    if (vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s()-> createTexture() failed for marbal.png !!! (ERROR CODE : %d)\n\n", __func__, vkResult);
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        return (vkResult);
    }
    else
    {
        fprintf(gpFile, "%s()-> createTexture() success for marbal.png \n\n", __func__);
    }
...
VkResult createVertexBuffer(void)
{
    ...
    vkBufferCreateInfo.pNext = NULL;
    vkBufferCreateInfo.flags = 0; // flags are used for scatterd / sparce buffer
    vkBufferCreateInfo.size = sizeof(cube_normal);
    vkBufferCreateInfo.usage = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
    ...
    // actual memeory mapped io
    memcpy(data, cube_normal, sizeof(cube_normal));
...
VkResult updateUniformBuffer(void)
{
   ...
   // update matricies
    ...
    myUniformData.lightAmbent[0] = 0.1f;
    myUniformData.lightAmbent[1] = 0.1f;
    myUniformData.lightAmbent[2] = 0.1f;
    myUniformData.lightAmbent[3] = 1.0f;
...
VkResult createPipeline(void)
{
    ...
    // for texture
    vkVertexInputBindingDescription_array[3].binding = 3; // corrospoing to layout = 3 in vertex shader
...
VkResult buildCommandBuffers(void)
{
    ...
    for(uint32_t i = 0; i < swapchainImageCount; i++)
    {
        ...
        vkCmdBindVertexBuffers(vkCommandBuffer_array[i], 2, 1, &vertexData_normal.vkBuffer, vkDeviceSize_offset_normal);
        ...
        vkCmdBindVertexBuffers(vkCommandBuffer_array[i], 3, 1, &vertexData_texcoord.vkBuffer, vkDeviceSize_offset_texcoord);
