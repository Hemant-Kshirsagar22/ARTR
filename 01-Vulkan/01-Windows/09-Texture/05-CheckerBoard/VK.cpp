...
VkDescriptorSet vkDescriptorSet_oblic = VK_NULL_HANDLE;
...
// for checkerboard
byte checkImage[CHECKERBOARD_HEIGHT][CHECKERBOARD_WIDTH][4];
...
VkResult initialize(void)
{
    ...
    if (vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s()-> createTexture() failed for checkerboard !!! (ERROR CODE : %d)\n\n", __func__, vkResult);
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        return (vkResult);
    }
    else
    {
        fprintf(gpFile, "%s()-> createTexture() success for checkerboard \n\n", __func__);
    }
...
void uninitialize(void)
{
    ...
    if(vkDescriptorPool)
    {
        ...
        vkDescriptorSet_oblic = VK_NULL_HANDLE;
    ...
    if(uniformData_oblic.vkDeviceMemory)
    {
        vkFreeMemory(vkDevice, uniformData_oblic.vkDeviceMemory, NULL);
        uniformData_oblic.vkDeviceMemory = VK_NULL_HANDLE;
        fprintf(gpFile, "%s()-> uniformData_oblic.vkDeviceMemory is freed\n", __func__);
    }

    // free uniform Buffer
    if(uniformData_oblic.vkBuffer)
    {
        vkDestroyBuffer(vkDevice, uniformData_oblic.vkBuffer, NULL);
        uniformData_oblic.vkBuffer = VK_NULL_HANDLE;
        fprintf(gpFile, "%s()-> uniformData_oblic.vkBuffer is freed\n", __func__);
    }
...
VkResult createVertexBuffer(void)
{
    ...
    vkBufferCreateInfo.pNext = NULL;
    vkBufferCreateInfo.flags = 0; // flags are used for scatterd / sparce buffer
    vkBufferCreateInfo.size = sizeof(rectangle_texcoord);
    vkBufferCreateInfo.usage = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
    ...
    // actual memeory mapped io
    memcpy(data, rectangle_texcoord, sizeof(rectangle_texcoord));
...
VkResult createTexture(void)
{
    ...
    int texture_width = CHECKERBOARD_WIDTH;
    int texture_height = CHECKERBOARD_HEIGHT;
...
VkResult createUniformBuffer(void)
{
    ...
    memset((void *)&uniformData_oblic, 0, sizeof(UniformData));
    ...
    vkResult = vkAllocateMemory(vkDevice, &vkMemeoryAllocateInfo, NULL, &uniformData_straight.vkDeviceMemory);

    if (vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s()-> vkAllocateMemory() for uniformData_straight.vkDeviceMemory failed !!!\n\n", __func__);
        return (vkResult);
    }
    else
    {
        fprintf(gpFile, "%s()-> vkAllocateMemory() call success for uniformData_straight.vkDeviceMemory\n\n", __func__);
    }
    ...
    // call vkCreateBuffer() vulkan api in the .vkBuffer member of our global struct.
    vkResult = vkCreateBuffer(vkDevice, &vkBufferCreateInfo, NULL, &uniformData_oblic.vkBuffer);
    ...
    vkGetBufferMemoryRequirements(vkDevice, uniformData_oblic.vkBuffer, &vkMemoryRequirements);
    ...
    vkResult = vkAllocateMemory(vkDevice, &vkMemeoryAllocateInfo, NULL, &uniformData_oblic.vkDeviceMemory);

    if (vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s()-> vkAllocateMemory() for uniformData_oblic.vkDeviceMemory failed !!!\n\n", __func__);
        return (vkResult);
    }
    else
    {
        fprintf(gpFile, "%s()-> vkAllocateMemory() call success for uniformData_oblic.vkDeviceMemory\n\n", __func__);
    }

    vkResult = vkBindBufferMemory(vkDevice, uniformData_oblic.vkBuffer, uniformData_oblic.vkDeviceMemory, 0);
    if (vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s()-> vkBindBufferMemory() failed !!!\n\n", __func__);
        return (vkResult);
...
VkResult updateUniformBuffer(void)
{
    ...
    // map uniform buffer
    void *data = NULL;
    vkResult = vkMapMemory(vkDevice, uniformData_straight.vkDeviceMemory, 0, sizeof(MyUniformData), 0, &data);
    ...
    translationMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(1.5f, 0.0f, -4.5f));
    ...
    // map uniform buffer
    data = NULL;
    vkResult = vkMapMemory(vkDevice, uniformData_oblic.vkDeviceMemory, 0, sizeof(MyUniformData), 0, &data);
    ...
    vkUnmapMemory(vkDevice, uniformData_oblic.vkDeviceMemory);
    return(vkResult);
}
...
VkResult createDescriptorSet(void)
{
    ...
    if (vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s()-> vkAllocateDescriptorSets() failed for vkDescriptorSet_straight (ERROR CODE : %d )!!!\n\n", __func__, vkResult);
        return (vkResult);
    }
    else
    {
        fprintf(gpFile, "%s()-> vkAllocateDescriptorSets() call success for vkDescriptorSet_straight\n\n", __func__);
    }
    ...
    // ---------------------- for oblic -----------------------
    vkResult = vkAllocateDescriptorSets(vkDevice, &vkDescriptorSetAllocateInfo, &vkDescriptorSet_oblic);
    ...
    vkDescriptorBufferInfo.buffer = uniformData_oblic.vkBuffer;
    vkDescriptorBufferInfo.offset = 0;
    vkDescriptorBufferInfo.range = sizeof(MyUniformData);
    ...
    vkWriteDescriptorSet_array[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    vkWriteDescriptorSet_array[0].pNext = NULL;
    vkWriteDescriptorSet_array[0].dstSet = vkDescriptorSet_oblic;
    vkWriteDescriptorSet_array[0].dstArrayElement = 0;
    vkWriteDescriptorSet_array[0].descriptorCount = 1;
    vkWriteDescriptorSet_array[0].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    vkWriteDescriptorSet_array[0].pBufferInfo = &vkDescriptorBufferInfo;
    ...
    vkWriteDescriptorSet_array[1].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    vkWriteDescriptorSet_array[1].pNext = NULL;
    vkWriteDescriptorSet_array[1].dstSet = vkDescriptorSet_oblic;
    vkWriteDescriptorSet_array[1].dstArrayElement = 0;
    vkWriteDescriptorSet_array[1].descriptorCount = 1;
    vkWriteDescriptorSet_array[1].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    vkWriteDescriptorSet_array[1].pBufferInfo = NULL;
...
VkResult buildCommandBuffers(void)
{
    ...
    for(uint32_t i = 0; i < swapchainImageCount; i++)
    {
        ...
        // ------------------ for oblic ---------------------------
        // bind our descriptor set to pipeline
        vkCmdBindDescriptorSets(vkCommandBuffer_array[i], VK_PIPELINE_BIND_POINT_GRAPHICS, vkPipelineLayout, 0, 1, &vkDescriptorSet_oblic, 0, NULL);
