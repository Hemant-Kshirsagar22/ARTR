...
VertexData vertexData_normal; // for light
...
VkResult createVertexBuffer(void)
{
    ...
    vkBufferCreateInfo.pNext = NULL;
    vkBufferCreateInfo.flags = 0; // flags are used for scatterd / sparce buffer
    vkBufferCreateInfo.size = sizeof(pyramid_normal);
    vkBufferCreateInfo.usage = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
    ...
    // actual memeory mapped io
    memcpy(data, pyramid_normal, sizeof(pyramid_normal));
...
VkResult createUniformBuffer(void)
{
    ...
    // memset our global uniformData struct.
...
VkResult createPipeline(void)
{
    ...
    // vertex input state
    VkVertexInputBindingDescription vkVertexInputBindingDescription_array[2]; // 0th for position, 1st for normal
    ...
    VkVertexInputAttributeDescription vkVertexInputAttributeDescription_array[2]; // 0th for position, 1st for normal
...
VkResult buildCommandBuffers(void)
{
    ...
    for(uint32_t i = 0; i < swapchainImageCount; i++)
    {
        ...
        vkCmdBindVertexBuffers(vkCommandBuffer_array[i], 0, 1, &vertexData_position.vkBuffer, vkDeviceSize_offset_array_position);
        ...
        vkCmdBindVertexBuffers(vkCommandBuffer_array[i], 1, 1, &vertexData_normal.vkBuffer, vkDeviceSize_offset_array_normal);
