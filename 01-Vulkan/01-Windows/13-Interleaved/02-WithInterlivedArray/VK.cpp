...
void uninitialize(void)
{
    ...
    if(vertexData_PCNT.vkDeviceMemory)
    {
        ...
        fprintf(gpFile, "%s()-> vertexData_PCNT.vkDeviceMemory is freed\n", __func__);
    }
    ...
    if(vertexData_PCNT.vkBuffer)
    {
        ...
        fprintf(gpFile, "%s()-> vertexData_PCNT.vkBuffer is freed\n", __func__);
    }
...
VkResult createVertexBuffer(void)
{
    ...
    // memset our global vertexData_PCNT struct.
    ...
    vkResult = vkAllocateMemory(vkDevice, &vkMemeoryAllocateInfo, NULL, &vertexData_PCNT.vkDeviceMemory);

    if (vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s()-> vkAllocateMemory() for vertexData_PCNT.vkDeviceMemory failed !!!\n\n", __func__);
        return (vkResult);
    }
    else
    {
        fprintf(gpFile, "%s()-> vkAllocateMemory() call success for vertexData_PCNT.vkDeviceMemory\n\n", __func__);
    }
    ...
    void *data = NULL;
    vkResult = vkMapMemory(vkDevice, vertexData_PCNT.vkDeviceMemory, 0, vkMemeoryAllocateInfo.allocationSize, 0, &data);
    if (vkResult != VK_SUCCESS)
    {
...
VkResult createPipeline(void)
{
    ...
    vkVertexInputAttributeDescription_array[0].offset = sizeof(float) * 0;
...
VkResult buildCommandBuffers(void)
{
    ...
    for(uint32_t i = 0; i < swapchainImageCount; i++)
    {
        ...
        memset((void *)vkDeviceSize_offset_PCNT, 0, sizeof(VkDeviceSize) * _ARRAYSIZE(vkDeviceSize_offset_PCNT));

        vkCmdBindVertexBuffers(vkCommandBuffer_array[i], 0, 1, &vertexData_PCNT.vkBuffer, vkDeviceSize_offset_PCNT);
