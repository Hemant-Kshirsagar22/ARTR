...
VkPhysicalDeviceFeatures vkPhysicalDeviceFeatures; // for tessellation shader
...
unsigned int uiNumberOfLineSegments = 1; // mapped with ubo's numberOfLineSegments
...
void uninitialize(void)
{
    ...
    // free tessellation evaluation shader
    if (vkShaderModule_tessellation_evaluation_shader)
    {
        ...
        fprintf(gpFile, "%s()-> vkShaderModule_tessellation_evaluation_shader is freed\n", __func__);
    }

    // free tessellation control shader
    if (vkShaderModule_tessellation_control_shader)
    {
        ...
        fprintf(gpFile, "%s()-> vkShaderModule_tessellation_control_shader is freed\n", __func__);
    }
...
VkResult createVulkanDevice(void)
{
    ...
    else
    {
        ...
        fprintf(gpFile, "%s()-> TESSELLATION SHADER NOT SUPPORTED !!!\n\n\n\n", __func__);
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        return(vkResult);
...
VkResult updateUniformBuffer(void)
{
    ...
    projectionMatrix = perspectivePojectionMatrix;

    myUniformData.mvpMatrix = projectionMatrix * viewMatrix * modelMatrix;

    // update line segments
    myUniformData.numberOfLineSegments.x = (float)uiNumberOfLineSegments;

    // update number of line strips
    myUniformData.numberOfLineStrips.x = (float)1;

    // update line color
    myUniformData.lineColor = glm::vec4(1.0f, 1.0f, 0.0f, 1.0f);

    // display changing number of line segment in windows caption bar
	TCHAR str[255];
	wsprintf(str, TEXT("HEMANT GANESH KSHIRSAGAR : Vulkan [numberOfLineSegment = %d]"), uiNumberOfLineSegments);
...
VkResult createShaders(void)
{
    ...
    szFileName = "./shaders/Shader.tesc.spv";
    fp = NULL;
    size = 0;
    ...
    if (vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s()-> vkCreateShaderModule() failed for tessellation control shader !!!\n\n", __func__);
        return (vkResult);
    }
    else
    {
        fprintf(gpFile, "%s()-> vkCreateShaderModule() call success for tessellation control shader\n\n", __func__);
    }
    ...
    szFileName = "./shaders/Shader.tese.spv";
    fp = NULL;
    size = 0;
    ...
    if (vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s()-> vkCreateShaderModule() failed for tessellation evaluation shader !!!\n\n", __func__);
        return (vkResult);
    }
    else
    {
        fprintf(gpFile, "%s()-> vkCreateShaderModule() call success for tessellation evaluation shader\n\n", __func__);
    }
...
VkResult createPipeline(void)
{
    ...
    vkVertexInputBindingDescription_array[0].stride = sizeof(float) * 2; // because our geometry is line hence we have only x and y coordinate
...
-----------------------------------------------------------------------------------------------
 ...
 C:\VulkanSDK\Vulkan\Bin\glslangValidator.exe -V -H -o ./shaders/Shader.tesc.spv ./shaders/Shader.tesc
 C:\VulkanSDK\Vulkan\Bin\glslangValidator.exe -V -H -o ./shaders/Shader.tese.spv ./shaders/Shader.tese
