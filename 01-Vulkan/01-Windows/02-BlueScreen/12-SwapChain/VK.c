...
VkSwapchainKHR vkSwapchinKHR = VK_NULL_HANDLE;
...
void uninitialize(void)
{
    ...
    if(vkSwapchinKHR)
    {
        vkDestroySwapchainKHR(vkDevice, vkSwapchinKHR, NULL);
        vkSwapchinKHR = VK_NULL_HANDLE;
...
VkResult createSwapchain(VkBool32 vsync)
{
    ...
    vkResult = vkCreateSwapchainKHR(vkDevice, &vkSwapchainCreateInfoKHR, NULL, &vkSwapchinKHR);
    if (vkResult != VK_SUCCESS)
    {
...
/**
 ...
 NOTE : when we create vulkanDevice it creates deviceQueue automatically and when we uninitlalize the vulkanDevice it will uninitialze the device queue.
 
 GET SURFACE FORMAT AND COLOR SPACE
 ----------------------------------
 1. call vkGetPhysicalDeviceSurfaceFormatKHR() first to retirve count of supported format.
 2. Declare and allocate array of VkSurfaceFormat Structure currosponding to above count, this structure has two members first VkFormat and  second VkColorSpaceKHR.
 3. call the same above function again but now to fill above array.
 4. Acording to the contains of above filled array dicide the surface color format and  surface color space.
 5. Free the above array
 
 PRESENT MODE
 -----------------
  1. call vkGetPhysicalDeviceSurfacePresentationModesKHR() first to retirve count of supported format.
  2. Declare and allocate araay of VkPresentationModeKHR Enum currospoding to above count.
  3. call above function again to full above array.
  4. Acording to the contains of above filled array dicide the persent mode.
  5. free the above array
 -----------------------------------------------------------------------------------------------
 
 STEPS FOR SWAPCHAIN
 -------------------
 1. Get Physical device surface supported color format and physical device surface supported color space useing previous step No 10.
 ...
 4. By using currentExtent.width and currentExtent.height members of above structure and compairing them with current width and height of window beside image width and image height of swap chain.
 5. Decide How we are going to use the swapchain images means whether we are going to store image data and use it letter(defered Rendering) or  we are going to use it as color attachment.
 ...
 8. Acoring to above data declare, memset and initialize VkSwapchainCreateInfo structure.
