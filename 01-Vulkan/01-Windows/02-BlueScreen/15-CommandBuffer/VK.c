...
/**
 ----------------------------------------------------------------------------------------------
 STEPS FOR COMMAND POOL
 -----------------------
1. declare and initialize VkCommandPoolCreateInfo sruct
2. call vkCreateCommandPool() to create the commandPool
3. destroy command pool using vkDestroyCommandPool()

 ----------------------------------------------------------------------------------------------
 STEPS FOR COMMAND BUFFER
 ------------------------
 1. Declare and initialize struct vkCommandBufferAllocateInfo
    NOTE : the number of command buffer are conventionally equal to the number of swapchain images
 2. Declare a command buffer array globally and allocate it to size of swapchain image count
 3. In a loop allocate each array buffer in above array by using vkAllocateCommandBuffer(), At a time of allocation of buffer will be empty letter we will fill it for comutation or rendering.
 4. In uninitialize() free each command buffer in a loop of size swapchainImageCount.
