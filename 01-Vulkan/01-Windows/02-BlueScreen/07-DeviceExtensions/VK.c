...
/*
 ...
 STEPS FOR printVKInfo
 -------------------------
 1. Remove Local declarations of physicalDeviceCout and physicalDeviceArray do it globaly from getPhysicalDevice().
 2. Accordingly remove physicalDeviceArray freeing block from if (if(bFound == TRUE)) statement and we will letter write freeing physicalDeviceArray block in printVKInfo().
 3. write printVKInfo() with following steps
    ...
    b. initialze this struct variable by calling vkGetPhysicalDeviceProperties() vulkan API.
    c. Print vulkan API Version using "apiVersionMember" member of above struct this require 3 vulkan macros.
    ...
    f. Print hexadecimal Vender id of device using "venderID" member of above struct.
    g. Print hexadecimal Device Id using "deviceID" member of above struct.
    * NOTE * For the sek of completeness we can repeat step 5 a to h from getPhysicalDevice() but now insted of assigning selected queue and selected device print whether this device support graphic bit, compute bit, transfer bit using if-else if-else if block, Similarly we also can repeat Device features from getPhysicalDevice() function and can print allaround 50+ device features includeing supporting tessilation shader and geometry shader.
    h. Free physcial device array here which we removed from if block (if(bFound == TRUE)) of getPhysicalDevice().

 -------------------------------------------------------------------------------------------------
 STEPS FOR INSTANCE EXTENSIONS
 -----------------------------
 using the similar steps of instance extensions we create device extensions, change api names,and variable names
