...
{
    // code
    animation_time = animation_time + 0.05;
}
...
VkResult createVertexBuffer(unsigned int mesh_width, unsigned int mesh_height, VertexData *pVertexData)
{
    ...
    vkBufferCreateInfo.size = sizeof(size);
...
VkResult prepareSignWaveForCPU(unsigned int mesh_width, unsigned int mesh_height, float anim_time)
{
    ...
    fillSignWaveArraysForCPU(width, mesh_height, anim_time);

    // map the buffers
    if(mesh_width = 1024 && height == 1024)
    {
    ...
    for(int i = 0; i < (int)mesh_width; i++)
    {
        for(int j = 0; j < (int)mesh_height, j++)
        {
            for(k = 0; k < 4; k++)
            {
                ...
                float w = signf(u * frequency + anim_time) * cosf(v * frequency + anim_time);
