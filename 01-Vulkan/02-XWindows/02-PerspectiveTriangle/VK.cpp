...
// VK_KHR_SURFACE_EXTENSION_NAME, VK_KHR_XLIB_SURFACE_EXTENSION_NAME and VK_EXT_DEBUG_REPORT_EXTENSION_NAME we are using these two macros and going to store in following array
...
VkResult fillInstanceExtensionNames(void)
{
    ...
    // 5. find whether above extension names contain our required two extentions (VK_KHR_SURFACE_EXTENSION_NAME macro of VK_KHR_surface, VK_KHR_XLIB_SURFACE_EXTENSION_NAME macro of VK_KHR_XLIB_surface) accordingly set two global variable.
    ...
    {
        vkResult = VK_ERROR_INITIALIZATION_FAILED; // return hardcoded failure
        fprintf(gpFile, "fillInstanceExtensionNames()-> VK_KHR_XLIB_SURFACE_EXTENSION_NAME NOT FOUND !!!\n\n");
        return (vkResult);
    }
    else
    {
        fprintf(gpFile, "fillInstanceExtensionNames()-> VK_KHR_XLIB_SURFACE_EXTENSION_NAME FOUND !!!\n\n");
    }
...
VkResult getSupportedSurface(void)
{
    ...
    // 3. Now Call vkCreateXlibSurfaceKHR() to create persentation suface object.
    vkResult = vkCreateXlibSurfaceKHR(vkInstance, &vkXlibSurfaceCreateInfoKHR, NULL, &vkSurfaceKHR);
    if (vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "getSupportedSurface()-> vkCreateXlibSurfaceKHR() FAILED !!!\n\n");
        return (vkResult);
    }
    else
    {
        fprintf(gpFile, "getSupportedSurface()-> vkCreateXlibSurfaceKHR() Success !!!\n\n");
    }
